      ...
      valuesContent   = <<-EOT
        externalServices:
          prometheus:
            ...
            basicAuth:
              username: "${local.prometheus_credentails_data.username}"
              password: ${local.prometheus_credentails_data.password}
          loki:
            host: https://logs-prod-eu-west-0.grafana.net
            basicAuth:
              username: "${local.loki_credentails_data.username}"
              password: ${local.loki_credentails_data.password}
          tempo:
            host: https://tempo-eu-west-0.grafana.net:443
            basicAuth:
              username: "${local.tempo_credentails_data.username}"
              password: ${local.tempo_credentails_data.password}
        cluster:
          name: ${var.name}
        metrics:
          enabled: true

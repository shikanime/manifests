      ...
      valuesContent = yamlencode({
        externalServices = {
          prometheus = {
            ...
            basicAuth = {
              username = local.prometheus_credentials_data.username
              password = local.prometheus_credentials_data.password
            }
          }
          loki = {
            host = "http://logs-prod-eu-west-0.grafana.net"
            basicAuth = {
              username = local.loki_credentials_data.username
              password = local.loki_credentials_data.password
            }
          }
          tempo = {
            host = "https://empo-eu-west-0.grafana.net:443"
            basicAuth = {
              username = local.tempo_credentials_data.username
              password = local.tempo_credentials_data.password
            }
          }
        }
        cluster = {

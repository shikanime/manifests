...
- `apps/`: user-facing applications and services
  ...
  - `apps/<app>/overlays/<cluster>*/`: cluster-specific overlays (often
    `*-tailnet`)
...
- `clusters/`: cluster-specific build entrypoints and compositions
  ...
  - `clusters/<cluster>/overlays/<overlay>/`: cluster build entrypoints
    (Kustomize)
- `skaffold.yaml`: render profiles pointing at cluster overlay entrypoints

## Secrets / Security (SOPS)

- Always use the strictest security settings available for all applications and
  deployments (e.g., enable authentication, restrict permissions).
- Encrypted files are stored as `*.enc.*` (examples: `.enc.env`,
  `config.enc.yaml`).
...
- Always add probes for workloads: `readinessProbe`, `livenessProbe`, and
  `startupProbe` (where applicable).
- Prefer Kustomize patches/components over duplicating manifests across
  overlays.
